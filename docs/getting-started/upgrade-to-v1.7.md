...
removal in and the regular upgrade flow handles them
...
- The next scan will re-index those files normally, into
 ...
 flagged via the regular rule engine (rule engine
...
- **–03** — column-resize state for the Files and
...
reversible **except for the quarantine column drop**.
