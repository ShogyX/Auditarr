...
`queue_optimization` action append items to a queue here for Stage 10's
...
`OptimizationItem` keyed by `(media_file_id, profile)`. Stage 10's
...
items is Stage 10. The Optimization page in the UI lists queued items
