#!/usr/bin/env bash
# Auditarr Docker installer.
#
# Use this script when you want Auditarr to run inside Docker.
# For bare-metal (LXC / VM / no Docker), use ./install-bare-metal.sh
# instead.
#
...
PYTHON
    ...
    if [[ ${#LIBRARY_PATHS[@]} -gt 0 ]]; then
        ...
        {
            echo "# Auto-generated by install-docker.sh"
            echo "# Adjust freely; re-running install-docker.sh will offer to regenerate."
            echo "services:"
            echo "  app:"
            echo "    volumes:"
            i=1
