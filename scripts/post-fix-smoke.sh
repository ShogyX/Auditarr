...
if grep -q '__version__ = "1.8.3"' "${REPO_ROOT}/backend/app/__init__.py" 2>/dev/null; then
    src_check "backend __version__ == 1.8.3 (Stage 16)" "ok"
else
    src_check "backend __version__ == 1.8.3 (Stage 16)" "version mismatch"
fi
if grep -q '"version": "1.8.3"' "${REPO_ROOT}/frontend/package.json" 2>/dev/null; then
    src_check "frontend package.json version == 1.8.3 (Stage 16)" "ok"
else
    src_check "frontend package.json version == 1.8.3 (Stage 16)" "version mismatch"
fi
