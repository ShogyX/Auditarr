...
app instance:
  ...
  6. GET ``/api/v1/media`` — empty page, but the endpoint
     answers.
  7. GET ``/api/v1/media/vocabulary`` — typed-builder
     vocabulary endpoint answers (empty vocabulary on a fresh
     install).
  8. GET ``/api/v1/rules`` — empty list, but the endpoint
     answers.
  9. GET ``/api/v1/dashboard/foreign-audio`` — v1.9 language
     surface answers with the default-empty shape.
 10. GET ``/api/v1/dashboard/incompatible-media`` — v1.9
     rule-flagged surface answers with the default-empty shape.
 11. Version check — assert ``__version__`` is ``1.9.0``.

If a real Docker stack is available, the test *may* bring it up;
we don't gate on that. The in-memory ASGI transport gives the same
surface coverage with none of the Docker setup, so this is the
deterministic path.
"""
...
async def test_release_smoke_full_walk(smoke_client: AsyncClient) -> None:
    ...
    # 9: /api/v1/dashboard/foreign-audio — v1.9 language surface.
    # Fresh install ⇒ count=0, sample_ids=[], default ["eng"]
    # preferences echoed back. The endpoint always returns a
    # structurally-complete payload so the client doesn't need
    # an error path for "no preferences configured" (the
    # "configure these" nudge in the UI is driven by empty
    # preference lists, not an error).
    fa = await smoke_client.get(
        "/api/v1/dashboard/foreign-audio", headers=headers
    )
    assert fa.status_code == 200, fa.text
    fa_body = fa.json()
    assert fa_body["count"] == 0
    assert fa_body["sample_ids"] == []
    assert fa_body["preferred_audio_languages"] == ["eng"]
    assert fa_body["preferred_subtitle_languages"] == ["eng"]

    # 10: /api/v1/dashboard/incompatible-media — v1.9
    # rule-flagged surface. Fresh install with no rules ⇒
    # count=0. The endpoint reports zero rather than a 404 so
    # the dashboard tile can self-hide cleanly.
    im = await smoke_client.get(
        "/api/v1/dashboard/incompatible-media", headers=headers
    )
    assert im.status_code == 200, im.text
    im_body = im.json()
    assert im_body["count"] == 0
    assert im_body["sample_ids"] == []
...
@pytest.mark.asyncio
async def test_release_version_is_1_9_0(smoke_client: AsyncClient) -> None:
    """The release artifact ships at 1.9.0 — verified at the
    Python ``__version__`` level (the source of truth) and via
    the application log on startup."""
    assert __version__ == "1.9.0", (
        f"expected __version__='1.9.0', got {__version__!r}"
    )
...
async def test_health_reports_version(smoke_client: AsyncClient) -> None:
    ...
    # we only assert the version field is present + 1.9.0.
    assert body.get("version") == "1.9.0", (
        f"health.version should be 1.9.0; got {body.get('version')!r}"
    )
